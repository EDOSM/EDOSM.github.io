...
let AuthComponent = class AuthComponent {
    ...
    onSubmit(form) {
        ...
        authObs.subscribe(resData => {
            const user_created = "User " + username + " Created Successfully";
            setTimeout(() => {
...
let AuthService = class AuthService {
    ...
    login(username, password) {
        return this.http
            ...
            this.handleAuthentication(resData['username'], resData['user_id'], resData['access_token'], +resData['expires_in']);
        }));
    }
    autoLogin() {
        ...
        const loadedUser = new _user_model__WEBPACK_IMPORTED_MODULE_6__["User"](userData.username, userData.id, userData._token, new Date(userData._tokenExpirationDate));
        if (loadedUser.token) {
    ...
    handleAuthentication(username, userId, token, expiresIn) {
        const expirationDate = new Date(new Date().getTime() + expiresIn * 1000);
        const new_user = new _user_model__WEBPACK_IMPORTED_MODULE_6__["User"](username, userId, token, expirationDate);
...
class User {
    constructor(username, id, _token, _tokenExpirationDate) {
